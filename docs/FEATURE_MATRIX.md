...
| Home | Continue, Inbox, new, local recommendations, recent favorites; empty sections hidden | Section order/visibility persisted |
...
| Google / Sync | Supabase Google OAuth path, RLS schema, opt-in whole-state revision sync, local logout preservation | Local-only mode is fully usable without configuration |
