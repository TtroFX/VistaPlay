...
export default function LibraryPage() {
  ...
  const [tab, setTab] = useState<Tab>('favorites'); const [folderName, setFolderName] = useState(''); const [folderParent, setFolderParent] = useState(''); const [selectedFolderId, setSelectedFolderId] = useState(''); const [selectedTagId, setSelectedTagId] = useState(''); const [tagName, setTagName] = useState('')
  ...
  const createSmart = () => {
    ...
    app.replaceState({ ...app.state, smartFolders: [...app.state.smartFolders, folder], revision: app.state.revision + 1, updatedAt: new Date().toISOString() }); setSmartName(''); setSmartConditions([]); setSmartGroups([])
  }
  return <div className="page"><div className="page-heading"><div><span className="eyebrow">YOUR COLLECTION</span><h1>Library</h1><p>Favorite・後で見る・Folder・Tagは相互排他ではありません。</p></div>{app.feature('statistics') && <button className="secondary-button" onClick={() => navigate('/statistics')}><BarChart3 />Statistics</button>}</div>
    ...
    {tab === 'folders' && <>
      ...
      <div className="folder-grid">{[...app.state.folders].sort((a, b) => Number(Boolean(a.parentId)) - Number(Boolean(b.parentId)) || Number(b.pinned) - Number(a.pinned)).map((folder) => {
        ...
        return <article className={`folder-card ${folder.parentId ? 'subfolder' : ''} ${selectedFolderId === folder.id ? 'selected' : ''}`} key={folder.id}><div className="folder-icon"><Folder /></div><button className="folder-open" onClick={() => setSelectedFolderId(folder.id)}><strong>{folder.name}</strong><span>{folder.videoIds.length}本 · {parent ? `${parent.name} / Subfolder` : 'Root'}</span></button><button className="icon-button" aria-label={`${folder.name}を${folder.pinned ? 'Pin解除' : 'Pin'}`} onClick={() => app.replaceState({ ...app.state, folders: app.state.folders.map((item) => item.id === folder.id ? { ...item, pinned: !item.pinned, updatedAt: new Date().toISOString() } : item), revision: app.state.revision + 1, updatedAt: new Date().toISOString() })}><Pin fill={folder.pinned ? 'currentColor' : 'none'} /></button><button className="icon-button danger-hover" aria-label={`${folder.name}を削除`} onClick={() => { if (!confirm('Folderを完全に削除しますか？動画自体は削除されません。')) return; if (selectedFolderId === folder.id || app.state.folders.some((item) => item.parentId === folder.id && item.id === selectedFolderId)) setSelectedFolderId(''); app.replaceState({ ...app.state, folders: app.state.folders.filter((item) => item.id !== folder.id && item.parentId !== folder.id), revision: app.state.revision + 1, updatedAt: new Date().toISOString() }) }}><Trash2 /></button></article>
      })}</div>
    ...
    {tab === 'smart' && <>
      <div className="smart-builder">
        ...
        <select value={smartField} onChange={(event) => setSmartField(event.target.value as SmartCondition['field'])}><option value="favorite">Favorite</option><option value="channel">Channel</option><option value="category">Category</option><option value="tag">Tag</option><option value="duration">Duration</option><option value="watchState">Watch State</option><option value="addedDate">Added age（日）</option><option value="publishedDate">Published age（日）</option></select>
      ...
      <div className="smart-folder-list">{app.state.smartFolders.map((folder) => <article className="smart-folder-card" key={folder.id}><div><Pin /><strong>{folder.name}</strong><span>{folder.groups?.length ?? 1} Group · {folder.conditions.length}条件 · Group間 {folder.operator.toUpperCase()}</span></div><p>{smartResults[folder.id]?.length ?? 0}件 — 条件から自動集計</p><div className="mini-video-row">{smartResults[folder.id]?.slice(0, 4).map((item) => <img key={item.videoId} src={item.thumbnail} alt="" />)}</div><button className="danger-text" onClick={() => confirm('Smart Folderを削除しますか？') && app.replaceState({ ...app.state, smartFolders: app.state.smartFolders.filter((item) => item.id !== folder.id), revision: app.state.revision + 1, updatedAt: new Date().toISOString() })}>Delete</button></article>)}</div>
    </>}
