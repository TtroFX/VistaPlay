...
function matchesCondition(video: VideoRef, condition: SmartCondition, state: PersistedAppState): boolean {
  ...
  switch (condition.field) {
    ...
    case 'addedDate': actual = progress?.updatedAt; break
    case 'publishedDate': actual = video.publishedAt; break
  }
...
export function evaluateSmartFolder(folder: SmartFolder, state: PersistedAppState): VideoRef[] {
  const videos = Object.values(state.videos)
  return videos.filter((video) => {
    const outcomes = folder.conditions.map((condition) => matchesCondition(video, condition, state))
